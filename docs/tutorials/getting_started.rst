...
To do that, we can use the :meth:`Chart.options <highcharts_gantt.chart.Chart.options>`
property. Having assembled our chart following the instructions above, ``my_chart``
already contains a :class:`HighchartsOptions <highcharts_gantt.options.HighchartsOptions>`
instance in the :meth:`Chart.options <highcharts_gantt.chart.Chart.options>` property.
You can access the :class:`LineSeries <highcharts_gantt.options.series.area.LineSeries>`
we created at :meth:`Chart.options.series <highcharts_gantt.options.HighchartsOptions.series>`,
and you can set any other options you need on 
:meth:`Chart.options <highcharts_gantt.chart.Chart.options>`.

For example, let's say we want to set the chart's title to "My First Chart". To do that,
we can configure the 
:meth:`Chart.options.title <highcharts_gantt.options.HighchartsOptions.title>` property
using either a :class:`Title <highcharts_gantt.options.title.Title>` instance, or a
...
call the :meth:`.display() <highcharts_gantt.chart.Chart.display>` method on 
``my_chart``:
...
:meth:`.download_chart() <highcharts_gantt.chart.Chart.download_chart>` method:
