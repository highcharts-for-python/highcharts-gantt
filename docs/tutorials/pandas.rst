...
These standard methods - with near-identical syntax - are available:
  ...
  * On the :class:`Chart <highcharts_gantt.chart.Chart>` class
  * On the :class:`options.data.Data <highcharts_gantt.options.data.Data>` 
    class
...
is to call :class:`Chart.from_pandas() <highcharts_gantt.chart.Chart.from_pandas>` like
so:
...
column to the resulting series' data points' :meth:`.x <highcharts_gantt.options.series.data.bar.BarData.x>`,
and to map the ``'2019'`` column to the :meth:`.y <highcharts_gantt.options.series.data.bar.BarData.y>` 
properties, respectively.

The net result is that ``my_chart`` contains one 
:class:`BarSeries <highcharts_gantt.options.series.bar.BarSeries>` whose 
:meth:`.data <highcharts_gantt.options.series.bar.BarSeries.data>` property contains a 
:class:`BarDataCollection <highcharts_gantt.options.series.data.bar.BarDataCollection>` instance populated
...
Now we're getting somewhere! First, we changed our series type to a :class:`ColumnSeries <highcharts_gantt.options.series.bar.ColumnSeries>` to make it (a little) easier to read. Then we  added a list of column names to the ``'y'`` key in the ``property_map``  argument. Each of those columns has now produced a *separate* :class:`ColumnSeries <highcharts_gantt.options.series.bar.ColumnSeries>` instance - but they're all still sharing the ``'Geographic Area'`` column as their :meth:`.x <highcharts_gantt.options.series.data.bar.BarData.x>` value.
  ...
  .. note::
    ...
    :exc:`HighchartsPandasDeserializationError <highcharts_gantt.errors.HighchartsPandasDeserializationError>`.
...
then it would use that column to set the :meth:`.id <highcharts_gantt.options.series.data.bar.BarData.id>` property of each data point.
...
:meth:`ColumnSeries.point_padding <highcharts_gantt.options.series.bar.ColumnSeries.point_padding>` property to a
...
:class:`HighchartsOptions <highcharts_gantt.options.HighchartsOptions>` configuration.`
...
the method call, we now set the :meth:`.container <highcharts_gantt.chart.Chart.container>` property
on ``my_chart``.
...
:meth:`.name <highcharts_gantt.options.series.bar.BarSeries.name>` values are populated from the ``'Geographic Area'``
column, while the data point :meth:`.x <highcharts_gantt.options.series.data.bar.BarData.y>` values come from each additional column (e.g. ``'2010'``, ``'2011'``, ``'2012'``, etc.)
  ...
  .. tip::
    ...
    For more information, please see:
      ...
      * :class:`Chart.from_pandas_in_rows() <highcharts_gantt.chart.Chart.from_pandas_in_rows>`
      * :class:`SeriesBase.from_pandas_in_rows() <highcharts_gantt.options.series.base.SeriesBase.from_pandas_in_rows>`
...
:class:`SeriesBase <highcharts_gantt.options.series.base.SeriesBase>` class. And they all
...
as :class:`Chart.from_pandas() <highcharts_gantt.chart.Chart.from_pandas>` *except for* ``options_kwargs`` and
...
Unlike when calling :meth:`Chart.from_pandas() <highcharts_gantt.chart.Chart.from_pandas>`, we
...
In this case, ``my_series`` now contains ten separate :class:`BarSeries <highcharts_gantt.options.series.bar.BarSeries>`
...
This will produce one :class:`LineSeries <highcharts_gantt.options.series.area.LineSeries>`
instance for each row in ``df``, ultimately producing a :class:`list <python:list>` of
57 :class:`LineSeries <highcharts_gantt.options.series.area.LineSeries>` instances.
...
:meth:`.load_from_pandas() <highcharts_gantt.options.series.base.SeriesBase.load_from_pandas>` method.
...
the ``series_index`` argument tells it to only use the 10th series generated.
  ...
  .. caution::
    ...
    While the :meth:`.load_from_pandas() <highcharts_gantt.options.series.base.SeriesBase.load_from_pandas>`
    method supports the same arguments as 
    :meth:`.from_pandas() <highcharts_gantt.options.series.base.SeriesBase.from_pandas>`, it expects that
